...
def result_auc(f1_result, f2_result, cishu):
    ...
    ##f5=f3+f3_1+f3_2
    f5 = f3 + f3_1 + f3_2
         # + f3_4 + f3_5 + f3_6 + f3_7 + f3_8 + f3_9
    f6 = f3 + f3_1 + f3_2
         # + f3_3 + f3_4 + f3_5 + f3_6 + f3_7 + f3_8 + f3_9
    ...
    for i in range(f4.shape[0]):
        for j in range(f4.shape[1]):
            f4[i, j] = f3[i, j] + f3_1[i, j] + f3_2[i, j]
                       # + f3_3[i, j] + f3_4[i, j] + f3_5[i, j] + f3_6[i, j] + f3_7[
...
def result_auc_1(f1_result, f2_result,gold, cishu):
...
def result_auc_1_2(f1_result, f2_result,gold, cishu):
    ...
    fpr3, tpr3, threshold3 = metrics.roc_curve(y_test, y_pre3)
    roc_auc3 = metrics.auc(fpr3, tpr3)
    precision3, recall3, _ = metrics.precision_recall_curve(y_test, y_pre3)
    aupr3 = metrics.auc(recall3, precision3)
    # acc3=accuracy_score(y_test, y_pre3)
    print(aupr3,'aupr2')
    print(roc_auc3,'f5')
...
def pre_A(f1_result, f2_result):

    A = result_auc(f1_result, f2_result, cishu=1)
    return A


def pre_A_1(f1_result, f2_result,gold):
    if gold.shape[0]<=5:
        A = result_auc_1(f1_result, f2_result,gold,cishu=1)
    else:
        A = result_auc_1_2(f1_result, f2_result,gold,cishu=1)
    # A = result_auc_1(f1_result, f2_result,gold,cishu=1)
    return A
# nn = 4
# cishu = 1
...
#     exe = pd.read_table('diffusion_clu/f1_clu' + str(i) + 'mse0.001_adam.csv', sep=',')
#     exe2 = pd.read_table('diffusion_clu/f2_clu' + str(i) + 'mse0.001_adam.csv', sep=',')
#     f1_result[:, :, i] = np.array(exe.iloc[:, 1:])
#     f2_result[:, :, i] = np.array(exe2.iloc[:, 1:])
#
#     result_auc(f1_result, f2_result, cishu)
