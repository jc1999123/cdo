...
def generate_dataset(data, args):
    ...
    ##data*train_ratio =batch_size的倍数
    train_dataset = MyDataset(data, 0, data.shape[0] * train_ratio, his_length, pred_length)

    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,drop_last=True)

    # valid_dataset = MyDataset(data, data.shape[0] * train_ratio, data.shape[0] * (train_ratio + valid_ratio),
    #                           his_length, pred_length)
...
def generate_dataset_test(data, args):
    """
    Args:
        data: input dataset, shape like T * N
        batch_size: int
    ...
    ##data*train_ratio =batch_size的倍数
    # print(data.shape)
    train_dataset = MyDataset(data, 0, data.shape[0], his_length, pred_length)

    train_dataloader = DataLoader(train_dataset, batch_size=batchsize1, shuffle=False)
